...
import { calculateDeal, formatMonthLabel } from './commission-calculations'
...
export function exportToCSV(deals: CommissionDeal[]): void {
  ...
  const headers = [
    ...
    'PU (€)',
    'PP (€)',
    'Frais entrée PU brut (%)',
    'Frais entrée PU net (%)',
    'Frais versement PP brut (%)',
    'Frais versement PP net (%)',
  ...
  const rows = calculated.map(d => [
    ...
    String(d.netEntryFeesRate),
    String(d.effectivePpPaymentFeesRate),
    String(d.netPpPaymentFeesRate),
